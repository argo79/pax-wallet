...
Refactored version - Cleaner architecture, same functionality
FIXED: Coinomi import (SECP256k1), Xaman numbers, Enum handling
FIXED: Standard compression for SECP256k1
"""
...
class XamanSecretNumbersBridge:
    ...
    def numbers_to_seed(self, numbers: List[str]) -> str:
        """Convert numbers to seed - SEMPRE Node.js per Xaman"""
        if not self._nodejs_available:
            raise ValueError("❌ Node.js is required for Xaman secret numbers")
        if self._use_python_fallback:
            raise ValueError("❌ Cannot use Python fallback for Xaman numbers")
    ...
    def _run_nodejs_script(self, numbers: List[str], return_field: str) -> str:
        """Run Node.js script to convert numbers - returns requested field"""
        ...
        // 🔥 PRIMA IL PACCHETTO VECCHIO (SECP256k1)
        try {{
            const pkg = require(path.join(modulePath, 'xrpl-secret-numbers'));
            Account = pkg.Account;
            useOld = true;
        }} catch (e) {{
            // POI IL PACCHETTO NUOVO (ED25519)
            try {{
    ...
    def _numbers_to_seed_nodejs(self, numbers: List[str]) -> str:
        return self._run_nodejs_script(numbers, "familySeed")
    
    def _numbers_to_address_nodejs(self, numbers: List[str]) -> str:
        return self._run_nodejs_script(numbers, "address")
...
class StellarManager:
    ...
    def set_network(self, network: str) -> None:
        """Set network with proper validation"""
        ...
        if network not in valid_networks:
            raise ValueError(f"Network not supported: {network}. Valid options: {valid_networks}")
        
        self.network = network
        server_url = STELLAR_NODE_URLS.get(network, STELLAR_NODE_URLS["testnet"])
        ...
        logger.info(f"✅ Stellar network set to: {network}")
...
class HybridXRPManager:
    ...
    def __init__(self, data_file: str = "wallet_data.json", password: str = None):
        ...
        logger.info(f"✅ HybridXRPManager initialized (not loaded)")
    
    @property
    def core(self):
        """Returns the Rust core instance"""
        return self._core
    
    @property
    def bridge(self) -> XamanSecretNumbersBridge:
        """Lazy initialization of Xaman bridge"""
    ...
    def initialize(self, password: str) -> bool:
        """Initialize manager with password: load and start core"""
        self._wallet_password = password
        
        # Load wallet data (decrypts DBs)
        if not self.load():
            print("ℹ️ No existing wallet data, creating new")
    ...
    def _decrypt_db_to_bytes(self, db_path: str) -> Optional[bytes]:
        """Decrypt a DB with decrypt_db_bytes() - RETURNS BYTES"""
        try:
            from core_wrapper import decrypt_db_bytes, is_encrypted_file
        except ImportError:
            return None
        
        if not self._wallet_password or not os.path.exists(db_path):
            return None
        
        try:
            if not is_encrypted_file(db_path):
            ...
            decrypted_bytes = decrypt_db_bytes(encrypted_str, self._wallet_password)
            return decrypted_bytes
        except Exception as e:
            logger.error(f"Error decrypting {db_path}: {e}")
            return None
    
    def _init_core(self) -> None:
        """Initialize Rust core - uses wallet_cli.db"""
        if not CORE_AVAILABLE or self._core_initialized:
            return
        ...
        try:
            ...
            db_path = DATA_DIR / "wallet_cli.db"
            temp_path = None
            
            if os.path.exists(db_path):
                decrypted_bytes = self._decrypt_db_to_bytes(str(db_path))
                if decrypted_bytes:
                    temp_file = tempfile.NamedTemporaryFile(delete=False, suffix='.db')
                    temp_file.write(decrypted_bytes)
                    temp_file.close()
                    temp_path = temp_file.name
                    print(f"🔐 DB decrypted to temp: {temp_path}")
                else:
                    temp_path = str(db_path)
            else:
                temp_file = tempfile.NamedTemporaryFile(delete=False, suffix='.db')
                temp_file.close()
                temp_path = temp_file.name
            ...
            self._core = create_core(temp_path)
            ...
            logger.info(f"✅ Core Rust initialized with wallet_cli.db")
        except Exception as e:
    ...
    # ============================================================
    # PRIVATE KEY DERIVATION HELPERS - STANDARD SECP256K1
    # ============================================================
    
    def _derive_secp256k1_from_mnemonic_coinomi(self, mnemonic: str, passphrase: str = "") -> Tuple[str, str, str]:
        """
        Deriva SECP256k1 keys from mnemonic usando lo STANDARD SECP256k1.
        ✅ USA LA COMPRESSIONE STANDARD DELLA LIBRERIA
        """
        if not self.mnemo.check(mnemonic):
            raise ValueError("Mnemonic non valida")
        
        # 1. Deriva seed BIP39
        seed_bytes = self.mnemo.to_seed(mnemonic, passphrase)
        
        # 2. Deriva con BIP32 path Coinomi
        bip32 = BIP32.from_seed(seed_bytes)
        private_key_bytes = bip32.get_privkey_from_path("m/44'/144'/0'/0/0")
        private_key_hex = private_key_bytes.hex()
        
        # 3. Deriva public key SECP256k1 con compressione STANDARD
        sk = ecdsa.SigningKey.from_string(private_key_bytes, curve=ecdsa.SECP256k1)
        vk = sk.get_verifying_key()
        
        # ✅ USA LA LIBRERIA PER LA COMPRESSIONE
    ...
    def _private_key_to_keypair_standard(self, private_key_hex: str) -> Tuple[str, str]:
        """
        Convert private key to public key and address usando lo STANDARD SECP256k1.
        ✅ USA LA COMPRESSIONE STANDARD DELLA LIBRERIA
        """
    ...
    def _derive_ed25519_from_mnemonic(self, mnemonic: str, passphrase: str = "") -> Tuple[bytes, str, str]:
        """
        Derive ED25519 keys from mnemonic using standard XRP method
        (This is for Xaman compatibility, NOT Coinomi)
        """
        import hashlib
        import nacl.signing
        from xrpl.wallet import Wallet as XRPWallet
        ...
        seed_bytes = self.mnemo.to_seed(mnemonic, passphrase)
        sha512_hash = hashlib.sha512(seed_bytes).digest()
        private_key_bytes = sha512_hash[:32]
        private_key_hex = private_key_bytes.hex()
        
        signing_key = nacl.signing.SigningKey(private_key_bytes)
        public_key_hex = signing_key.verify_key.encode().hex()
        
        temp_wallet = XRPWallet(public_key=public_key_hex, private_key=private_key_hex)
        address = temp_wallet.classic_address
        
        return private_key_bytes, public_key_hex, address
    
    def _derive_seed_from_private_key(self, private_key_hex: str) -> str:
        """Derive XRP seed (sEd...) from an ED25519 private key."""
        from xrpl.core import keypairs
        from xrpl.constants import CryptoAlgorithm

        private_key_bytes = bytes.fromhex(private_key_hex)
        entropy = private_key_bytes.hex()[:32]
        seed = keypairs.generate_seed(entropy=entropy, algorithm=CryptoAlgorithm.ED25519)
        return seed
    ...
    def _clean_numbers_input(self, raw_input: str) -> List[str]:
        """Clean and extract numbers from input string"""
        if not raw_input:
            return []
    ...
    def _bip39_to_private_key(self, phrase: str, passphrase: str = "") -> str:
        """Derive private key from BIP39 mnemonic (SECP256k1) - Coinomi method"""
    ...
    def _validate_secp256k1(self, key_hex: str) -> bool:
        """Check if private key is valid for SECP256k1 (key < order)"""
        order = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141
        return int(key_hex, 16) < order
    
    def _numbers_to_seed_fallback(self, numbers: List[str]) -> str:
        """Fallback conversion for numbers to seed"""
        seed_bytes = b""
        for num_str in numbers:
            num = int(num_str)
    ...
    def _get_xrpl_client(self):
        """Get XRPL client for current network"""
        from xrpl.clients import JsonRpcClient
        url = XRP_NODE_URLS.get(self.network, XRP_NODE_URLS["testnet"])
        return JsonRpcClient(url)
    
    def _init_stellar(self) -> None:
        """Initialize stellar manager if needed"""
    ...
    def set_network(self, network: str) -> None:
        ...
        logger.info(f"✅ Network set to: {network}")
    
    def set_crypto(self, crypto_type: str) -> None:
        """Set cryptocurrency type"""
        if isinstance(crypto_type, CryptoType):
            crypto_type = crypto_type.value
        crypto_type = str(crypto_type).upper().strip()
    ...
    def create_new_wallet_bip39(self, passphrase: str = "", strength: int = 128) -> Dict[str, Any]:
        """Crea nuovo wallet BIP39 compatibile con Coinomi/Ledger (SECP256k1 BIP44)."""
        ...
        self.crypto_type = "XRP"

        private_key_hex, public_key_hex, address = self._derive_secp256k1_from_mnemonic_coinomi(
            self.seed_phrase, passphrase
        )
        ...
        return {
            ...
            "first_public_key": public_key_hex,
        }
    ...
    def create_new_wallet_numbers(self) -> Dict[str, Any]:
        """Create new wallet with Xaman secret numbers"""
    ...
    def import_wallet(self, seed_input: Union[str, List[str]], 
        ...
        """Import wallet - FIXED VERSION"""
        # Normalizza input_type a stringa
        if isinstance(input_type, SeedType):
            input_type = input_type.value
        elif hasattr(input_type, 'value'):
            input_type = input_type.value
        else:
        ...
        # Ora input_type è SEMPRE una stringa
        if input_type == SeedType.BIP39.value:
            if self.crypto_type == "XLM":
                return self._import_bip39_as_stellar(seed_input, passphrase)
            else:
                # ✅ USA SECP256k1 PER COINOMI
                return self._import_bip39_coinomi(seed_input, passphrase)
    ...
    def _import_bip39_coinomi(self, seed_phrase: str, passphrase: str = "") -> Dict[str, Any]:
        """
        Import mnemonic Coinomi - USA LA LIBRERIA STANDARD!
        """
        if not self.mnemo.check(seed_phrase):
            raise ValueError("❌ Invalid seed phrase!")
        
        print("🔍 Importing Coinomi wallet (SECP256k1 + BIP44)...")
        
        # 1. Deriva seed e private key
        seed_bytes = self.mnemo.to_seed(seed_phrase, passphrase)
        bip32 = BIP32.from_seed(seed_bytes)
        private_key_bytes = bip32.get_privkey_from_path("m/44'/144'/0'/0/0")
        private_key_hex = private_key_bytes.hex()
        
        # 2. ✅ USA LA LIBRERIA STANDARD PER LA COMPRESSIONE
        sk = ecdsa.SigningKey.from_string(private_key_bytes, curve=ecdsa.SECP256k1)
        vk = sk.get_verifying_key()
        
        # ✅ METODO STANDARD DELLA LIBRERIA
        public_key_hex = vk.to_string("compressed").hex()
        address = keypairs.derive_classic_address(public_key_hex)
        ...
        self.passphrase = passphrase
        self.crypto_type = "XRP"
        self.base_private = private_key_bytes
        self.base_seed_xrp = None
        self._correct_address = address
        
        self.save()
        
        print(f"✅ Coinomi wallet imported successfully!")
        print(f"   Address: {address}")
    ...
    def _import_bip39_ed25519(self, seed_phrase: str, passphrase: str = "") -> Dict[str, Any]:
        """Import mnemonic with ED25519 (Xaman compatible)"""
        if not self.mnemo.check(seed_phrase):
            raise ValueError("❌ Invalid seed phrase!")
        ...
        private_key_bytes, public_key_hex, address = self._derive_ed25519_from_mnemonic(
            seed_phrase, passphrase
        )
        private_key_hex = private_key_bytes.hex()
        
        self.base_private = private_key_bytes
        self.base_seed_xrp = self._derive_seed_from_private_key(private_key_hex)
        self._correct_address = address
    ...
    def _import_bip39_as_stellar(self, seed_phrase: str, passphrase: str = "") -> Dict[str, Any]:
        """Import BIP39 mnemonic as Stellar wallet"""
        if not STELLAR_AVAILABLE:
            raise ImportError(f"stellar-sdk not installed: {STELLAR_IMPORT_ERROR}")
    ...
    def _import_private_key(self, private_key_hex: str) -> Dict[str, Any]:
        """
        Import from private key (supports both ED25519 and SECP256K1)
        ✅ USA LA COMPRESSIONE STANDARD DELLA LIBRERIA
        """
        try:
            # 64 characters (32 bytes) → ED25519 o SECP256k1
            if len(private_key_hex) == 64:
                ...
                # Fallback SECP256K1 con compressione STANDARD della libreria
                try:
                ...
                raise ValueError(f"Invalid private key length: {len(private_key_hex)}")
                
        except Exception as e:
            raise ValueError(f"Invalid private key: {e}")
    
    def _import_xrp_seed(self, xrp_seed: str) -> Dict[str, Any]:
        """Import from XRP seed - supports both ED25519 and SECP256K1"""
        try:
            xrp_seed = xrp_seed.strip()
            if ' ' in xrp_seed:
                raise ValueError("XRP seed cannot contain spaces")
    ...
    def _import_numbers(self, numbers: Union[str, List[str]]) -> Dict[str, Any]:
        """Import from Xaman secret numbers"""
        ...
        try:
            # USA SEMPRE IL BRIDGE NODE.JS PER XAMAN
            # MAI il fallback Python per i numeri!
    ...
    def validate_seed(self, seed_input: Union[str, List[str]]) -> Dict[str, Any]:
        """Validate seed input and return info"""
        input_type = self.detect_input_type(seed_input)
        result = {
            "valid": False,
            "type": input_type,
    ...
    def get_wallet(self, keyword: str = "default", index: int = 0) -> XRPWallet:
        """
        Returns XRP wallet ready to sign.
        ✅ USA LA COMPRESSIONE STANDARD DELLA LIBRERIA
        """
        if self.crypto_type == "XLM":
        ...
        # Se abbiamo seed phrase BIP39 - usa SECP256k1 (Coinomi standard)
        if self.seed_phrase and self.seed_type == SeedType.BIP39.value:
            private_key_hex, public_key_hex, _ = self._derive_secp256k1_from_mnemonic_coinomi(
                self.seed_phrase, self.passphrase
            )
            return XRPWallet(
        ...
        # Se abbiamo XRP seed (sEd...) - lascia che xrpl determini l'algoritmo
        elif self.base_seed_xrp:
            return XRPWallet.from_seed(self.base_seed_xrp)
        
        # Se abbiamo private key diretta - usa compressione STANDARD della libreria
        elif self.base_private:
            private_key_hex = self.base_private.hex()
            
            # Prova SECP256k1 con compressione STANDARD della libreria
            try:
                sk = ecdsa.SigningKey.from_string(self.base_private, curve=ecdsa.SECP256k1)
                vk = sk.get_verifying_key()
            ...
            try:
                import nacl.signing
                signing_key = nacl.signing.SigningKey(self.base_private)
                public_key_hex = signing_key.verify_key.encode().hex()
                return XRPWallet(
                    public_key=public_key_hex,
                    private_key=private_key_hex,
            ...
            raise ValueError("❌ No key available to create wallet")
    
    def _get_xrp_wallet(self, keyword: str = "default", index: int = 0) -> XRPWallet:
        """
        Get XRP wallet (legacy SECP256k1 for derived keys)
        ✅ USA LA COMPRESSIONE STANDARD DELLA LIBRERIA
        """
        ...
        # ✅ USA LA COMPRESSIONE STANDARD DELLA LIBRERIA
        public_key, _ = self._private_key_to_keypair_standard(private_key_hex)
        return XRPWallet(
    ...
    def get_wallet_info(self, keyword: str = "default", index: int = 0) -> WalletInfo:
        ...
        # RICALCOLA INDIRIZZO DALLA PRIVATE KEY (UNICA LOGICA)
        address = wallet.classic_address
        if self.base_private:
            try:
                _, address = self._private_key_to_keypair_standard(self.base_private.hex())
            except:
                pass
    ...
    def batch_derive_addresses(self, keywords: List[str], count: int = 5) -> Dict[str, List[WalletInfo]]:
        """Batch derive addresses for multiple keywords"""
        results = {}
    ...
    def get_derived_by_keyword(self, keyword: str) -> List[WalletInfo]:
        """Get derived wallets by keyword"""
    ...
    def get_trustlines(self, force_refresh: bool = False) -> List[Dict]:
        """Get trustlines for current wallet"""
    ...
    def _set_xrp_trustline(self, asset_code: str, issuer: str, limit: float = None) -> Dict:
        ...
        try:
            ...
            if limit == 0:
                limit_value = "0"
                flags = 0x00020000
                print(f"🗑️ Removing trustline for {asset_code} (flags=0x00020000)")
            else:
                limit_value = str(limit) if limit else "1000000000"
                flags = 0x00020000
                print(f"📤 Creating trustline for {asset_code}")
    ...
    def _decode_currency_hex(self, currency_hex: str) -> str:
        """Decode currency hex to readable string"""
    ...
    def reset(self) -> None:
        ...
        logger.info("✅ Wallet reset complete")
    ...
    def save(self) -> None:
        ...
        if self._core_initialized and self._core:
            try:
                self._core = None
                self._core_initialized = False
                print("🔐 Core closed for DB encryption")
            except:
                pass
        ...
        if self._wallet_password:
            ...
            if os.path.exists(db_path):
                try:
                    from core_wrapper import encrypt_db_bytes
                    with open(db_path, 'rb') as f:
                        db_bytes = f.read()
                    encrypted = encrypt_db_bytes(db_bytes, self._wallet_password)
                    if isinstance(encrypted, str):
                        ...
                        with open(db_path, 'wb') as f:
                            f.write(encrypted)
                    print(f"🔐 Encrypted wallet_cli.db")
                except Exception as e:
                    print(f"⚠️ Error encrypting wallet_cli.db: {e}")
        ...
        try:
                ...
                if os.path.exists(db_path):
                    try:
                        ...
                        if is_encrypted_file(db_path):
                            ...
                            print(f"🔐 Decrypted wallet_cli.db")
                    except Exception as e:
                        print(f"⚠️ Error decrypting wallet_cli.db: {e}")
            ...
            if base_private_hex:
                ...
                if self.crypto_type == "XLM":
                    ...
                    # Per Coinomi usa SECP256k1 con compressione STANDARD
                    private_key_hex, public_key_hex, address = self._derive_secp256k1_from_mnemonic_coinomi(
                        self.seed_phrase, self.passphrase
                    )
                    self.base_private = bytes.fromhex(private_key_hex)
    ...
    def close(self) -> None:
        """Close core and clean temporary files"""
        ...
        if hasattr(self, '_temp_db_path') and self._temp_db_path:
            try:
                if os.path.exists(self._temp_db_path):
                    ...
                    print(f"🗑️ Removed temp DB: {self._temp_db_path}")
            except:
                pass
...
def create_manager(data_file: str = "wallet_data.json", 
                  crypto_type: str = "XRP", 
                  network: str = "testnet",
                  password: str = None) -> HybridXRPManager:
    """Factory function to create a configured manager"""
    manager = HybridXRPManager(data_file, password=password)
...
if __name__ == "__main__":
    manager = HybridXRPManager("test_wallet.json", password="test123")
    manager.initialize("test123")
