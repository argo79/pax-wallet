...
class HistoryView(ListViewWithDetail):
    ...
    def _display_transactions(self, transactions):
        ...
        header = f"{'#':<3} {'Data/Ora':<20} {'Tipo':<10} {'Importo':<18} {'Fee':<10} {'Da/A':<30} {'Memo':<25}"
        self.tx_list.addItem("=" * 130)
        ...
        for idx, tx_data in enumerate(transactions, 1):
            ...
            line = f"{idx:<3} {date_str[:19]:<20} {direction:<10} {amount_str:<18} {fee_str:<10} {da_a[:28]:<30} {memo_str:<25}"
    ...
    def _parse_direction(self, tx, address):
        ...
        if destination == address:
            return "RICEVUTO", f"Da: {sender[:26]}"
        elif sender == address:
            return "INVIATO", f"A: {destination[:26]}"
        else:
            return "ALTRO", f"{sender[:13]}...→{destination[:13]}..."
