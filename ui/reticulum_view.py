    ...
    def update_status(self):
        ...
        try:
            ...
            if status.get("success"):
                ...
                use_tor = s.get('use_tor', False)
                internet_on = s.get('internet_on', True)
                
                # TOR SENZA IP
                self.tor_label.setText(f"🧅 ON" if use_tor else f"🧅 OFF")
                
                # INTERNET CON IP
                if internet_on:
                    self.internet_label.setText(f"🌐 ON - IP: {public_ip}")
                else:
                    self.internet_label.setText(f"📡 OFF")
